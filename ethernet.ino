...
char server[] = "192.168.31.79";                               // Адрес запрашиваемого сервера

IPAddress ip(192, 168, 31, 242);                                // Задаю статический IP-адрес на тот случай, если у DHCP выдать IP-адрес не получится
...
void setup()                                                    // Функция, задающая настройки
{
  ...
  if (client.connect(server, 8080))                             // Если подключение установлено, сообщаю об этом в COM-порт
  {
    ...
    client.println("Host: 192.168.31.79:8080");
